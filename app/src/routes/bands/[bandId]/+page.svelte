<script lang="ts">
	...
	>([]);
	let templates = $state<
		{ id: string; name: string; revision_date?: string; event_name?: string }[]
	>([]);
	let persons = $state<
		{
			id: number;
	...
	async function load() {
		...
		templates = await getTemplates(bandId);
	...
	async function handleCreateFromTemplate(templateId: string) {
		showNewPlotDialog = false;
		const newId = await createPlotFromTemplate(templateId, bandId);
		goto(`/bands/${bandId}/plots/${newId}?new=1`);
	}

	async function handleDuplicate(plotId: string) {
		const newId = await duplicatePlot(plotId, bandId);
		plots = [
			...plots,
			...(await db.query<{
				id: string;
				...
				revision_date: string;
				event_name: string | null;
			}>('SELECT id, name, revision_date, event_name FROM stage_plots WHERE id = ?', [newId]))
		];
	}

	async function handleSaveAsTemplate(plotId: string) {
		await createTemplateFromPlot(plotId, bandId);
		templates = await getTemplates(bandId);
	}
				...
				<div class="grid auto-rows-fr grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">
					{#each plots as plot (plot.id)}
						<ContextMenu.Root>
							...
							<ContextMenu.Content
								class="z-50 min-w-[160px] rounded-xl border border-border-primary bg-surface p-1 shadow-lg"
							>
								<ContextMenu.Item
									class="flex w-full cursor-pointer items-center rounded-lg px-3 py-2 text-sm text-text-primary transition hover:bg-surface-hover"
									onSelect={() => handleDuplicate(plot.id)}
								>
									Duplicate
								</ContextMenu.Item>
								<ContextMenu.Item
									class="flex w-full cursor-pointer items-center rounded-lg px-3 py-2 text-sm text-text-primary transition hover:bg-surface-hover"
									onSelect={() => handleSaveAsTemplate(plot.id)}
								>
									Save as Template
								</ContextMenu.Item>
								<ContextMenu.Separator class="my-1 h-px bg-border-primary" />
								<ContextMenu.Item
									class="flex w-full cursor-pointer items-center rounded-lg px-3 py-2 text-sm text-red-600 transition hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-900/20"
									onSelect={() => (deletePlotTarget = { id: plot.id, name: plot.name })}
								>
									Delete
								</ContextMenu.Item>
							</ContextMenu.Content>
				...
				>
					...
					<div class="mt-5 grid gap-2">
						...
						{#if templates.length > 0}
							<div class="mt-2 mb-1 text-xs font-medium tracking-wide text-text-tertiary uppercase">
								From Template
							</div>
							{#each templates as template (template.id)}
								...
								>
									...
									<div>
										...
										<div class="text-xs text-text-secondary">Use as starting point</div>
									</div>
								</button>
							{/each}
						{/if}
