...
import { insertMany, runInTransaction } from '$lib/db/batch';
...
export async function duplicateBand(bandId: string): Promise<string | null> {
	...
	return runInTransaction(async () => {
		...
		await insertPersonsForBand(newBandId, persons);
		...
		if (plots.length > 0) {
			...
			const musicianRows: unknown[][] = [];
			for (const musician of musicians) {
				const newPlotId = plotIdMap.get(musician.plot_id);
				if (!newPlotId) continue;
				musicianRows.push([newPlotId, musician.name, musician.instrument]);
			}
			await insertMany('musicians', ['plot_id', 'name', 'instrument'], musicianRows);
		}
		...
		if (gigs.length > 0) {
			...
			if (setlists.length > 0) {
				...
				for (const setlistSong of setlistSongs) {
					...
					setlistSongRows.push([newSetlistId, newSongId, setlistSong.position, setlistSong.notes]);
				}
				await insertMany(
					'setlist_songs',
