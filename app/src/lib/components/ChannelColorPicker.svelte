...
<Popover.Root bind:open>
	...
	<Popover.Portal>
		<Popover.Content
			class="z-50 rounded-lg border border-border-primary bg-surface p-3 shadow-xl"
			sideOffset={4}
			align="start"
		>
			<div class="mb-2 text-xs font-medium text-text-secondary">Channel {channelNum} Color</div>
			<!-- Normal colors row -->
			<div class="mb-2 grid grid-cols-8 gap-1.5">
				{#each normalColors as color (color.id)}
					<button
						...
						class="h-7 w-7 rounded-md border-2 transition-all hover:scale-110 focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 focus:outline-none {currentColorId ===
						color.id
			...
			{#if invertedColors.length > 0}
				<div class="mb-1 text-[10px] text-text-secondary">Inverted</div>
				<div class="grid grid-cols-8 gap-1.5">
					{#each invertedColors as color (color.id)}
						<button
							type="button"
							onclick={() => handleSelect(color.id)}
							class="h-7 w-7 rounded-md border-2 border-dashed transition-all hover:scale-110 focus:ring-2 focus:ring-blue-500 focus:ring-offset-1 focus:outline-none {currentColorId ===
							color.id
								? 'border-white ring-2 ring-blue-500'
								: 'border-transparent'}"
							style="background-color: {color.hex};"
